...
def check_axis_intersect(
    that: tuple[SupportNumber, SupportNumber], z: SupportNumber
):
    x, y = that
    return y >= z >= x
...
def is_collision(
    that: list[tuple[SupportNumber, SupportNumber]],
    other: list[SupportNumber],
) -> bool:
...
def calcdist(axis: tuple[SupportNumber, SupportNumber]):
    return math.sqrt(functools.reduce(operator.sub, axis) ** 2)


def middledist(axis: tuple[SupportNumber, SupportNumber]):
    dist = calcdist(axis) / 2

    return axis[0] + dist


def middleaxis(axis: tuple[SupportNumber, SupportNumber]):
    dist = calcdist(axis) / 2
    middle = axis[0] + dist
